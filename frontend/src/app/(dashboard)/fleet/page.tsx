...
export default function FleetPage() {
  ...
  const mintToken = async () => {
    try { const r = await client.mintJoinToken(); setJoinToken(r.join_token); } catch { /* ignore */ }
  };

  const copyToken = () => {
    if (joinToken) { navigator.clipboard?.writeText(joinToken); setCopied(true); setTimeout(() => setCopied(false), 1500); }
  };
    ...
    <div className="p-8 max-w-7xl mx-auto h-full flex flex-col text-white">
        ...
        {type === "byoc" && (
          <div className="mt-4 pt-4 border-t border-white/5">
            <div className="flex items-center justify-between gap-3 flex-wrap">
              <p className="text-sm text-zinc-400 flex items-center gap-2"><KeyRound className="w-4 h-4" /> BYOC daemons register with a single-use join token.</p>
              <button onClick={mintToken} className="text-sm bg-white/5 border border-white/10 hover:bg-white/10 rounded-lg px-3 py-1.5">Generate join token</button>
            </div>
            {joinToken && (
              <div className="mt-3 flex items-center gap-2 bg-black/40 border border-white/10 rounded-lg px-3 py-2">
                <code className="text-xs text-green-300 truncate flex-1">{joinToken}</code>
                <button onClick={copyToken} className="text-zinc-400 hover:text-white">{copied ? <Check className="w-4 h-4 text-green-400" /> : <Copy className="w-4 h-4" />}</button>
              </div>
            )}
          </div>
        )}
      ...
      {fleets.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          {fleets.map(f => (
            <div key={f.id} className="glass-panel p-4 border border-white/10 rounded-xl flex items-center gap-3">
