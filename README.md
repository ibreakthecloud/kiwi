...
Every PR modifying the codebase must also keep this README updated. If no update is necessary, add the `skip-readme-check` label to the PR. (Note: Health checks are available at `/healthz` and `/readyz`.) (Note: The `/api/v1/planner/plan` endpoint supports idempotent submissions via the `Idempotency-Key` header.)